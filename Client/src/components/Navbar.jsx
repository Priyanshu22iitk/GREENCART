...
const Navbar = () => {
  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
      ...
      {!user ?  (<button onClick={() => setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-green-500 hover:bg-green-700 transition text-white rounded-full">
  Login
</button>) :(<div className='relative group'>
    <img className="w-10 " src={assets.profile_icon} alt="" />
    <ul className='hidden group-hover:block absolute right-0 bg-white shadow border border-gray-200 py-2.5 w-30 rounded-md text-sm z-40' >
        <li onClick={() => navigate('/my-orders')} className='px-4 py-2 hover:bg-gray-100'>
            My Orders
        </li>
  ...
  <button onClick={() => open ? setOpen(false) : setOpen(true)} aria-label="Menu" className="">
      ...
      <div className={`${open ? 'flex' : 'hidden'} sm:hidden absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm`}>
        ...
        {!user ? (
          ...
          }} className="cursor-pointer px-6 py-2 mt-2 bg-green-500 hover:bg-green-700 transition text-white rounded-full text-sm">
            Login
          </button>
        ) : (
          <button  onClick={logout} className="cursor-pointer px-6 py-2 mt-2 bg-green-500 hover:bg-green-700 transition text-white rounded-full text-sm">
            Logout
          </button>
        )}
      </div>
