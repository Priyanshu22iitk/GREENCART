...
const Cart = () => {
    ...
    // Wait until products and cartItems are ready to prepare cart view
    const getCart = () => {
        let tempArray = [];
    ...
    // Get address after user is fetched
    ...
    // Get cart items when products or cartItems change
    useEffect(() => {
        if (products.length > 0 && Object.keys(cartItems).length > 0) {
            getCart();
        } else {
            setCartArray([]); // Clear cart if cart is empty
        }
    }, [products, cartItems]);

    // Get user address once user is loaded
    useEffect(() => {
        if (!loadingUser && user) {
            getUserAddress();
        }
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        ...
                                        <select onChange={e => updateCartItem(product._id, Number(e.target.value))} value={cartItems[product._id]} className='outline-none'>
                                            {Array(cartItems[product._id] ? cartItems[product._id] : 9).fill('').map((_, index) => (
